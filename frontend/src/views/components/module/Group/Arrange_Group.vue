<template>
  ...
  <vs-row vs-type="flex" vs-justify="end">
    <vs-col class="arrangeForm2" vs-justify="center" vs-w="12" vs-align="center" vs-lg="3" vs-xs="3" vs-sm="3">
      <vs-input label="배정할 조 번호를 입력해주세요" :color="color" v-model="grp_number"/>
    </vs-col>
    <vs-col vs-justify="center" vs-w="12" vs-align="center" vs-lg="6" vs-xs="12" vs-sm="6">
      <div class="button-container">
         <vs-button class="button-form" :color="color" type="filled" @click="arrangeGroups">팀 등록하기</vs-button>
      </div>
    </vs-col>
  </vs-row>
  </div>
...
methods:{
    arrangeGroups() {
      ...
      this.$http.post('/api/admin/group/arrange', {
        ssm_user : this.selected,
        ssm_new_team : this.grp_number
      }, config)
      .then(res => {
        alert('조 배정에 성공하였습니다.')
      })
